...
document.getElementById('flipRow').addEventListener('click', function(e) {
    if (e.target.id === 'flipColors') return;
    var cb = document.getElementById('flipColors');
    cb.checked = !cb.checked;
});

chrome.storage.local.get(['activePieceSkin', 'activeBoardSkin', 'flipColors'], function(res) {
    selectOption('pieceSkin', res.activePieceSkin || 'custom1');
    selectOption('boardSkin', res.activeBoardSkin || 'default');
    document.getElementById('flipColors').checked = res.flipColors || false;
});
...
document.getElementById('save').addEventListener('click', function() {
    ...
    var flipColors = document.getElementById('flipColors').checked;

    var pieceSkin = pieceEl ? pieceEl.dataset.value : 'custom1';
    var boardSkin = boardEl ? boardEl.dataset.value : 'default';

    chrome.storage.local.set({ activePieceSkin: pieceSkin, activeBoardSkin: boardSkin, flipColors: flipColors }, function() {
